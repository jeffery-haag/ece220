...
/* This code creates a floorplan for a circuit. There are some simple functions that just get data about the current cell. init_slicing_tree creates a tree with all the elements for the circuit on it, recursively. traverse_postfix
 * travels through the tree recursively, and gets all the data to create an expression for the circuit. get_total_resource gets all the resource for the circuit. swap_module swaps out modules for leaf nodes, while swap_topology
 * swaps whole nodes.
...
// Procedure: floorplan
// The major procedure of the floorplan. The procedure consists of the following major steps:
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t * aparentleft = NULL;
  node_t * bparentleft = NULL;
 
  if(a->parent->left == a) aparentleft = a;
  if(b->parent->left == b) bparentleft = b;
 
  //Set parent nodes
  a->parent = b->parent;
  b->parent = temp;
 
  //Set child nodes of parents
  if (bparentleft != NULL) a->parent->left = a;
  else a->parent->right = a;
  if (aparentleft != NULL) b->parent->left  = b;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression)
{
  ...
  {
    expression[counter1].module=ptr->module;
    counter1++;
    return;
  }
  if (ptr->left !=NULL)
  {
    dumb=nth;//(&(++(*nth)))
    *dumb=(*nth+1);
    postfix_traversal(ptr->left,dumb , expression);
  }   
  if (ptr->right !=NULL)
  {    dumb=nth;
    *dumb=(*nth+1);
    postfix_traversal(ptr->right,dumb, expression);
  }   
 
  if(ptr->module==NULL)expression[counter1].cutline=ptr->cutline;
  else expression[counter1].module=ptr->module;
  counter1++;
...
int get_total_resource(node_t* ptr)
{
  ...
  int sum=0;
  while (ptr->left != NULL) //Iterate through all nodes
  {
    sum = sum + ptr->right->module->resource; //Add current resource
    ptr = ptr->left;
  }
...
node_t* init_slicing_tree(node_t* par, int n)
{
  ...
  node_t * parent = (node_t*)malloc(sizeof(node_t)); //Malloc parent node
  parent = NULL;
 
  if (n == (num_modules-1)) //Create final node
  {
  ...
  //Recursion
  par->left = init_slicing_tree(par->left, n+1);
